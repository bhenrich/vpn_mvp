...
async fn load_jwks() -> anyhow::Result<Arc<JwkSet>> {
	...
	let jwks = if let Ok(path) = std::env::var("JWKS_JSON_PATH") {
		...
		let body = reqwest::get(url).await.context("fetching JWKS")?.text().await.context("reading JWKS body")?;
...
async fn admit(State(state): State<AdmissionState>, Json(req): Json<AdmitRequest>) -> axum::response::Result<Json<serde_json::Value>> {
	...
	let jwk = state.jwks.keys.iter().find(|k| k.common.key_id.as_deref() == Some(&kid))
	...
	let mut conn = state.redis.get_async_connection().await.map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
	...
	// purge expired
	let _: () = redis::cmd("ZREMRANGEBYSCORE").arg(&user_key).arg("-inf").arg(now).query_async(&mut conn).await
		.map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
	let _: () = redis::cmd("ZREMRANGEBYSCORE").arg(&device_key).arg("-inf").arg(now).query_async(&mut conn).await
		.map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;

	// counts
	let user_count: isize = redis::cmd("ZCARD").arg(&user_key).query_async(&mut conn).await
		.map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
	let device_count: isize = redis::cmd("ZCARD").arg(&device_key).query_async(&mut conn).await
		.map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
	...
	// add session
	let _: () = redis::cmd("ZADD").arg(&user_key).arg(expires).arg(&claims.jti).query_async(&mut conn).await
		.map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
	let _: () = redis::cmd("ZADD").arg(&device_key).arg(expires).arg(&claims.jti).query_async(&mut conn).await
		.map_err(|_| axum::http::StatusCode::INTERNAL_SERVER_ERROR)?;
...
pub async fn run_admission_server() -> anyhow::Result<()> {
	...
	let max_active = std::env::var("MAX_ACTIVE_CONNECTIONS").ok().and_then(|v| v.parse().ok()).unwrap_or(5);
...
async fn reap_expired(client: redis::Client) -> anyhow::Result<()> {
	...
	let keys: Vec<String> = redis::cmd("KEYS").arg("active:*").query_async(&mut conn).await?;
	let now = chrono::Utc::now().timestamp() as f64;
	for key in keys {
		let _: () = redis::cmd("ZREMRANGEBYSCORE").arg(&key).arg("-inf").arg(now).query_async(&mut conn).await?;
	}
	Ok(())
}
