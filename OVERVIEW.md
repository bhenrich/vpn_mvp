...
This repository contains everything required to build a privacy-first, multi-platform VPN product. The monorepo intentionally co-locates desktop clients, control-plane services, node software, tooling, and documentation so we can evolve the system with tight feedback loops, shared contracts, and consistent automation.
...
2. Run `make bootstrap` (or `scripts/bootstrap.sh`) to install toolchains and pre-commit hooks.
