...
def _coerce_psycopg_url(url: str) -> str:
	# Ensure Alembic uses psycopg v3 driver even if a plain postgresql:// is provided
	return "postgresql+psycopg://" + url[len("postgresql://") :] if url.startswith("postgresql://") else url


def run_migrations_offline() -> None:
	url = _coerce_psycopg_url(settings.DATABASE_URL)
	context.configure(
		url=url,
...
def run_migrations_online() -> None:
	...
	configuration["sqlalchemy.url"] = _coerce_psycopg_url(settings.DATABASE_URL)
